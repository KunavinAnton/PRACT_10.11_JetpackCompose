...
@Composable
fun AddTaskFragment(navController: NavController){
    Surface(
        modifier = Modifier.fillMaxSize(),
        color = Green
    ) {
        Column(verticalArrangement = Arrangement.SpaceBetween) {
            GeneralActivityHead(navController ,"Добавить задачу", R.drawable.back_button, 28, 58, 18, NavigationItems.List)
            Column(modifier = Modifier.size(428.dp, 328.dp), horizontalAlignment = Alignment.CenterHorizontally){
                AddTextField(hint = "Заголовок задачи", sizeWidth = 318.dp, sizeHeight = 48.dp)
                ...
                Row(modifier = Modifier.size(318.dp, 178.dp).padding(top = 8.dp)) {
                    AddTextField(hint = "Описание задачи", sizeWidth = 318.dp, sizeHeight = 148.dp, singleLine = false)
                }
            }
